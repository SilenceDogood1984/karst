...
- `--as MODEL:ID` on `bin/rails karst:verify` and `bin/rails karst:reproduce` — a human-only way to run a probe as one specific existing record instead of a sampled one (e.g. `karst:verify GET /admin/imports/123 --as User:72`). Resolved exclusively through the same `Identity.resolve` lookup the panel's own **Test as** button already uses, so a record outside a configured principal source never resolves; there is no `constantize`/`find` bypass. Cannot be combined with `--anonymous`, and is not exposed by either MCP tool — an agent can pick a request, never a principal.
