...
- `Karst::Access::PrincipalSampler` — a bounded candidate-selection step ahead of `Access::Sweep`. It resolves application-authored candidate populations first, then materializes one recent-N pool with a single `LIMIT` query. When no population contributes a candidate, generic boolean/enum/nullable-foreign-key/low-cardinality schema sampling runs only as an in-memory fallback over that pool. Retained explicit dimensions are also evaluated over the pool for compatibility. Its total query volume is one pool query plus at most one bounded query per configured population; no discovery query, `COUNT`, unbounded scan, or live subquery is used. It only selects candidates and never executes a route, so `Access::Sweep` remains the sole behavioral-evidence contract.
